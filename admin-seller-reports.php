...
// Handle removing reported item from feed
...
$stmt = $pdo->prepare("
    ...
    VALUES (?, 'deleted_product', ?, ?)
");
$stmt->execute([$admin_id, "Deleted reported product ID: $product_id", $_SERVER['REMOTE_ADDR']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['notify_reported'])) {
    ...
    $stmt = $pdo->prepare("SELECT * FROM user_reports WHERE report_id = ?");
    $stmt->execute([$report_id]);
    $report = $stmt->fetch();
    
    // Message to reported user
    $msg_reported = "CASE #" . $case_number . "\n\nWe have received a report from a seller regarding your account. You have been flagged in our system. If you receive two more reports, your account will be permanently removed from NEXUS.\n\nPlease review our Terms of Service and Community Guidelines.\n\n- NEXUS Safety Team";
    ...
    $stmt = $pdo->prepare("SELECT COUNT(*) as report_count FROM user_reports WHERE reported_id = ? AND status = 'reviewed'");
    $stmt->execute([$reported_id]);
    $report_count = $stmt->fetch()['report_count'];
    
    $success = "Case #" . $case_number . " processed. User has " . $report_count . " report(s).";
}
...
// Get reported items (products that have been reported via user_reports linking to products)
$reported_items = [];
try {
    $stmt = $pdo->prepare("
        SELECT DISTINCT p.*, sp.store_name,
               ur.reason as reported_reason,
               ur.description as reported_description,
               ur.created_at as reported_at,
               (SELECT email FROM nexus_users WHERE user_id = ur.reporter_id) as reporter_email
        FROM user_reports ur
        JOIN nexus_users u_rep ON ur.reporter_id = u_rep.user_id
        JOIN products p ON ur.reported_id = p.seller_id
        JOIN seller_profiles sp ON p.seller_id = sp.profile_id
        WHERE u_rep.user_type = 'seller' AND ur.status = 'pending'
        AND p.is_reported = 1 AND p.listing_status = 'active'
...
$stmt = $pdo->prepare("
    SELECT ur.*, 
           ...
           rd_user.user_type as reported_type,
...
$stmt = $pdo->prepare("
    ...
    LEFT JOIN buyer_profiles bp2 ON rd_user.user_id = bp2.user_id
    LEFT JOIN seller_profiles sp2 ON rd_user.user_id = sp2.user_id
    WHERE u_rep.user_type = 'seller' AND ur.status IN ('reviewed', 'dismissed')
...
<html lang="en">
<head>
    <title>Admin | Seller Reports</title>
    <meta charset="UTF-8">
    ...
    <main class="admin-main">
        <div class="admin-container">
            <div class="page-header">
                <h1><i class="fas fa-store"></i> Seller Reports</h1>
                <p>Review and manage reports submitted by sellers</p>
            </div>
                    ...
                    <div class="reported-item-card">
                        <div class="reported-item-header">
                            <div>
                                ...
                                <p>Seller: <?php echo htmlspecialchars($item['store_name']); ?> | Reported by: <?php echo htmlspecialchars($item['reporter_email']); ?> | <?php echo date('d M Y H:i', strtotime($item['reported_at'])); ?></p>
                            </div>
                        </div>
            ...
            <?php if (empty($pending_reports)): ?>
                <div class="empty-state"><i class="fas fa-check-circle" style="font-size: 48px; margin-bottom: 16px;"></i><p>No pending seller reports</p></div>
            <?php else: ?>
                <?php foreach ($pending_reports as $report): 
                    $case_number = 'NEX-S' . str_pad($report['report_id'], 6, '0', STR_PAD_LEFT);
                ?>
                    <div class="report-card">
                        ...
                        <div class="report-body">
                            <div class="info-grid">
                                <div class="info-item"><label>Reporter (Seller)</label><div class="value"><?php echo htmlspecialchars($report['reporter_name']); ?></div></div>
                                <div class="info-item"><label>Reported User</label><div class="value"><?php echo htmlspecialchars($report['reported_name']); ?> (<?php echo ucfirst($report['reported_type']); ?>)</div></div>
                            ...
                            <div class="action-buttons">
                                ...
                                <form method="post" style="display: inline;" onsubmit="return confirm('Notify both parties?');">
                ...
                ?>
                    <div class="report-card">
                        <div class="report-header">
                            <div>
                                <h3>Case #<?php echo $case_number; ?></h3>
                                <p><?php echo htmlspecialchars($report['reporter_name']); ?> (Seller) reported <?php echo htmlspecialchars($report['reported_name']); ?></p>
                            </div>
